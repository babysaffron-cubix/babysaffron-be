...
public partial class ShoppingCartController : BaseNopWebApiFrontendController
{
    ...
    public ShoppingCartController(IAttributeParser<CheckoutAttribute, CheckoutAttributeValue> checkoutAttributeParser,
    ...
    {
        var model = request.Model.FromDto<EstimateShippingModel>() ?? new EstimateShippingModel();
        ...
        var orderedCart = await itemsWithNewQuantity
            .OrderByDescendingAwait(async cartItem =>
                ...
                (cartItem.NewQuantity > cartItem.Item.Quantity && cartItem.Product != null && (await _shoppingCartService
                    .GetProductsRequiringProductAsync(cartCopy, cartItem.Product)).Any()))
            .ToListAsync();
