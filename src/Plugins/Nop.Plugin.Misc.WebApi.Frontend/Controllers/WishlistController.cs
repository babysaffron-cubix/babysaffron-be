...
public partial class WishlistController : BaseNopWebApiFrontendController
{
    ...
    public WishlistController(CatalogSettings catalogSettings,
            ...
            if (remove)
                ...
                foreach (var formKey in form.Keys)
                    if (formKey.Equals($"itemquantity{sci.Id}", StringComparison.InvariantCultureIgnoreCase))
                    {
                        ...
                        {
                            var currentSciWarnings = await _shoppingCartService.UpdateShoppingCartItemAsync(await _workContext.GetCurrentCustomerAsync(),
                                sci.Id, sci.AttributesXml, sci.CustomerEnteredPrice,
                                sci.RentalStartDateUtc, sci.RentalEndDateUtc,
                                newQuantity);
                            innerWarnings.Add(sci.Id, currentSciWarnings);
                        }
