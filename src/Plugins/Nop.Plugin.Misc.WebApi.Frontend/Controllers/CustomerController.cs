...
public partial class CustomerController : BaseNopWebApiFrontendController
{
            ...
            switch (attribute.AttributeControlType)
            {
                ...
                case AttributeControlType.Checkboxes:
                    if (!StringValues.IsNullOrEmpty(attributeValues))
                        foreach (var attributeValue in attributeValues.Split(_separator, StringSplitOptions.RemoveEmptyEntries))
    ...
    public virtual async Task<IActionResult> PasswordRecoverySend([FromBody] PasswordRecoveryModelDto model)
    {
        ...
        var customer = await _customerService.GetCustomerByEmailAsync(passwordRecoveryModel.Email);
        ...
        [FromQuery, Required] Guid customerGuid)
    {
        ...
        var model = passwordRecoveryConfirmModel.FromDto<PasswordRecoveryConfirmModel>();
    ...
    public virtual async Task<IActionResult> Info([FromBody] CustomerUpdateRequestModelDto request)
    {
        var customer = await _workContext.GetCurrentCustomerAsync();
        ...
        [FromQuery, Required] string contentType)
    {
        ...
        if (!_customerSettings.AllowCustomersToUploadAvatars)
            return BadRequest($"The setting {nameof(_customerSettings.AllowCustomersToUploadAvatars)} = false");

        if (contentType != null && !contentType.Equals("image/jpeg") && !contentType.Equals("image/gif"))
            return BadRequest(await _localizationService.GetResourceAsync("Account.Avatar.UploadRules"));
