...
public partial class DownloadController : BaseNopWebApiFrontendController
{
    ...
    public virtual async Task<IActionResult> WebPdfInvoice(int orderId)
    {
        ...
        return new FileContentResult(bytes, MimeTypes.ApplicationPdf) { FileDownloadName = string.Format(await _localizationService.GetResourceAsync("PDFInvoice.FileName"), order.CustomOrderNumber) + ".pdf" };
    }
