...
public partial class ProductController : BaseNopWebApiFrontendController
{
    ...
    public virtual async Task<IActionResult> GetProductDetails(int productId, [FromQuery] int updateCartItemId = 0)
    {
        ...
        foreach (var picture in pictures)
        {
            ...
            var pictureModelDto = productDetailsModel.PictureModels.FirstOrDefault(x => x.FullSizeImageUrl == fullSizeImageUrl);

            if (pictureModelDto == null)
                continue;

            var (imageUrl, _) = await _pictureService.GetPictureUrlAsync(picture, pictureSize);
            pictureModelDto.ImageUrl = imageUrl;
        }
    ...
    public virtual async Task<IActionResult> EstimateShipping([FromBody] BaseModelDtoRequest<ProductEstimateShippingModelDto> request)
    {
        ...
        var model = request.Model.FromDto<ProductDetailsModel.ProductEstimateShippingModel>() ?? new ProductDetailsModel.ProductEstimateShippingModel();

        switch (_shippingSettings.EstimateShippingCityNameEnabled)
        {
            case false when string.IsNullOrEmpty(model.ZipPostalCode):
                errors.Add(await _localizationService.GetResourceAsync("Shipping.EstimateShipping.ZipPostalCode.Required"));
                break;
            case true when string.IsNullOrEmpty(model.City):
    ...
    public virtual async Task<IActionResult> ProductReviewsAdd([FromBody] ProductReviewsModelDto model, int productId)
    {
        ...
        var productReviewsModel = await _productModelFactory.PrepareProductReviewsModelAsync(product);
        ...
        var result = productReviewsModel.ToDto<ProductReviewsModelDto>();

        result.AddProductReview.SuccessfullyAdded = true;
        if (!isApproved)
            result.AddProductReview.Result = await _localizationService.GetResourceAsync("Reviews.SeeAfterApproving");
        else
            result.AddProductReview.Result = await _localizationService.GetResourceAsync("Reviews.SuccessfullyAdded");
    ...
    /// <param name="helpfulness">Indicator if the review was helpful</param>
