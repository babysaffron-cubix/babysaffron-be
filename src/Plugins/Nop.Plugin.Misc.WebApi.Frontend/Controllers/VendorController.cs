...
public partial class VendorController : BaseNopWebApiFrontendController
{
                ...
                case AttributeControlType.ReadonlyCheckboxes:
                    {
                        ...
                        foreach (var selectedAttributeId in attributeValues
                                     ...
                                     .Select(v => v.Id)
                                     .ToList())
                            attributesXml = _vendorAttributeParser.AddAttribute(attributesXml, attribute,
    ...
    public virtual async Task<IActionResult> Info([FromBody] InfoRequest request, [FromQuery][Required] string contentType)
    {
        ...
        //vendor attributes
        var vendorAttributesXml = await ParseVendorAttributesAsync(request.Form);
        var attributeWarnings = await _vendorAttributeParser.GetAttributeWarningsAsync(vendorAttributesXml);

        if (attributeWarnings.Any())
            return BadRequest(attributeWarnings);
