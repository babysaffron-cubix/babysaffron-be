...
#pragma warning disable CS0618
        CreateDtoMap<ProductAttributeValue, ProductAttributeValueDto>(map => map.ForMember(p => p.AttributeValueType, options => options.Ignore()).ForMember(p => p.PictureId, options => options.Ignore()));
        CreateDtoMap<ProductAttributeCombination, ProductAttributeCombinationDto>(entityMappingExpression: map => map.ForMember(p => p.PictureId, options => options.Ignore()));
#pragma warning restore CS0618

        CreateDtoMap<PredefinedProductAttributeValue, PredefinedProductAttributeValueDto>();

        CreateDtoMap<ProductAttributeCombinationPicture, ProductAttributeCombinationPictureDto>();

        CreateDtoMap<ProductAttributeValuePicture, ProductAttributeValuePictureDto>();
    ...
    protected virtual void CreateMessagesMaps()
    {
        ...
        CreateDtoMap<EmailAccount, EmailAccountDto>(entityMappingExpression: map =>
            map.ForMember(p => p.EmailAuthenticationMethod, options => options.Ignore()));
    ...
    protected virtual void CreateCustomerMaps()
    {
        ...
        CreateDtoMap<Customer, CustomerDto>(entityMappingExpression: map =>
