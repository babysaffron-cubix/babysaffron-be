...
public partial class PermissionRecordController : BaseNopWebApiBackendController
{
    ...
    {
        var permissionRecord = (await _permissionService.GetAllPermissionRecordsAsync()).FirstOrDefault(p => p.Id == model.Id);
    ...
    {
        var permissionRecord = (await _permissionService.GetAllPermissionRecordsAsync()).FirstOrDefault(p => p.SystemName.Equals(systemName));
