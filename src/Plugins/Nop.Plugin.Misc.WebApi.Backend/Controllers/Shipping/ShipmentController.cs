...
public partial class ShipmentController : BaseNopWebApiBackendController
{
        ...
        [FromQuery] int pageSize = int.MaxValue)
    {
        var shipments = await _shipmentService.GetAllShipmentsAsync(vendorId, warehouseId, shippingCountryId,
            shippingStateId, shippingCounty, shippingCity, trackingNumber, loadNotShipped, loadNotReadyForPickup, loadNotDelivered, orderId, createdFromUtc, createdToUtc, pageIndex, pageSize);
    ...
    {
        var shipmentIds = ids.Split(";").Where(s => int.TryParse(s, out _)).Select(int.Parse).ToArray();
