...
public partial class ShoppingCartItemController : BaseNopWebApiBackendController
{
    ...
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public virtual async Task<IActionResult> ClearShoppingCart(int customerId,
        [FromQuery] int storeId = 0)
    {
        ...
        await _shoppingCartService.ClearShoppingCartAsync(customer, storeId);
    ...
    public virtual async Task<IActionResult> GetShoppingCartItemAttributeWarnings(int customerId,
        ...
        [FromQuery] bool ignoreBundledProducts = false,
    ...
    {
        var cartIds = cartItemIds.Split(";").Where(s => int.TryParse(s, out _)).Select(int.Parse)
            .ToList();
        if (!cartIds.Any())
            return BadRequest();
