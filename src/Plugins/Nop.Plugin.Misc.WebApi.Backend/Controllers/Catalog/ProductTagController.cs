...
public partial class ProductTagController : BaseNopWebApiBackendController
{
    ...
    [HttpPost]
    [ProducesResponseType(typeof(int), StatusCodes.Status200OK)]
    public virtual async Task<IActionResult> InsertProductTag([FromBody] ProductTagDto productTag)
    {
        var query = from pt in _productTagRepository.Table
                    where pt.Name == productTag.Name
                    select pt;
        ...
        if (entity == null)
        {
            entity = productTag.FromDto<ProductTag>();

            await _productTagRepository.InsertAsync(entity);
        }
