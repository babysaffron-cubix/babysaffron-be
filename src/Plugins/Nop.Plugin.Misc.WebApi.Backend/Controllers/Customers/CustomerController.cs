...
public partial class CustomerController : BaseNopWebApiBackendController
{
    ...
    /// </summary>
    [HttpDelete("onlyWithoutShoppingCart")]
    [ProducesResponseType(typeof(string), StatusCodes.Status404NotFound)]
    ...
    [ProducesResponseType(typeof(string), StatusCodes.Status404NotFound)]
    [ProducesResponseType(typeof(int), StatusCodes.Status200OK)]
    public virtual async Task<IActionResult> GetCustomerTaxDisplayType(int customerId)
    {
        var customer = await _customerService.GetCustomerByIdAsync(customerId);

        if (customer == null)
            return NotFound($"Customer Id={customerId} is not found");

        var taxDisplayType = await _customerService.GetCustomerTaxDisplayTypeAsync(customer);

        return Ok((int)taxDisplayType);
    }
